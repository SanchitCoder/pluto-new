...
function ThreeDScrollTriggerRowImpl({
  ...
  const BlockContent = useMemo(
    () => (
      <div className="inline-flex shrink-0 whitespace-normal" style={{ contain: 'paint' }}>
        {childrenArray}
      </div>
    ),
    [childrenArray]
  ...
  const measure = () => {
    ...
    if (block && block.scrollWidth > 0) {
      ...
      const needed = Math.max(3, Math.ceil(containerWidth / w) + 2);
      setNumCopies(needed);
    }
  };
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'w-full overflow-x-clip overflow-y-visible whitespace-nowrap py-1',
        className
      )}
      {...props}
    >
